...
async def process_analysis(analysis_id: str, file_path: Path, filename: str, file_type: str):
    ...
    try:
        ...
        # Step 3: Generate AI-powered Frida scripts based on ACTUAL code
        await manager.send_progress(analysis_id, {
            "status": "generating",
        ...
        # Generate script for each UNIQUE detection
        unique_detections = {}
        for d in detections:
            key = f"{d.class_name}.{d.method_name}"
        ...
        for idx, (key, detection) in enumerate(unique_detections.items()):
            ...
            try:
                ...
                await manager.send_progress(analysis_id, {
                    "status": "generating",
                    "message": f"Generated {idx+1}/{len(unique_detections)} scripts...",
        ...
        await db.analyses.update_one(
            ...
            {"$set": {
                "status": "completed",
                "completed_at": datetime.utcnow(),
                "detections": [d.dict() for d in list(unique_detections.values())],
